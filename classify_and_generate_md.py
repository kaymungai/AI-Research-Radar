...
def classify_and_generate_md(md_path: str, knowledge_path: str):
    """
    读取知识库和md内容，调用zhipuai，返回分类和新md内容，并写入新目录。
    """
    # 读取知识库内容
    ...
    # 构造prompt
    prompt = f"""
你是一个AI模型分类与总结专家，并且可以访问arXiv文章的链接地址。请根据下面的“模型分类知识库”内容，判断下方md文件描述的模型属于哪个分类，并用如下md格式输出：
# [此处填当前模型md文件对应知识库中的分类名，不用包含[]符号]]

# [模型名称] - [arXiv 标题]

**模型功能**：
[请访问下方md内容中的arXiv链接，结合论文实际内容，总结模型的主要功能]

**arXiv 文章链接**：
[arXiv 链接](https://arxiv.org/abs/XXXX)

**作者/团队**：
[请访问下方md内容中的模型论文地址：，填写作者姓名或团队名称]

**发表日期**：
[请访问下方md内容中的模型论文地址：，填写论文的首次提交或发表日期]
